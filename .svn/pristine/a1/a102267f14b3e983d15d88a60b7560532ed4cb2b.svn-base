...
<html>
    <head>
        ...
        <script src="../js/Cal-Basic-2.0-min.js">  </script>
        <script src="../js/cal.support.js">  </script>
        <script src="../js/cal.default.template.js">  </script> 
		<script type="text/javascript" src="js/mybic.js"></script> 
        <title>PHP Calendar Demo</title> 
		<style>
		.calcredits{
...
*/
	                        var startdate= evObj._startTime.toStandardFormat();	
							var inputDate = new Date(startdate);
							var todaysDate = new Date();
							
							if( (new Date() < new Date(startdate)) || (inputDate.setHours(0,0,0,0) == todaysDate.setHours(0,0,0,0)))
							{
							jQuery("#defaultNewEventTemplate").find("#updateEventBtn").show().end()
							}else{
							jQuery("#defaultNewEventTemplate").find("#updateEventBtn").hide().end()
							}
...
jQuery("#defaultNewEventTemplate").find("#eventName").val(evObj.eventName).end()
							...
							/*.find("#Update").attr("href", "http://localhost/stylecal.php/?&date="+ evObj._startTime.toStandardFormat() + "&starttime="+ evObj.startTime + "&endtime=" + evObj.endTime + "&service=" + evObj.eventName + "&comment=" + evObj.eventDesc);*/
							.find("#Update").attr("href", "<?php echo SITEURL; ?>/services/stylecal.php/?id="+evId);
