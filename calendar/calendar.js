...
var rtlDefaults = {
	...
	//dayNamesShort: ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'],
	  ...
	  monthNames: ['ינואר','פברואר','מרץ','אפריל','מאי','יוני','יולי','אוגוסט','ספטמבר','אוקטובר','נובמבר','דצמברh'],
...
var fc = $.skCalendar = { version: "1.6.4" };
var fcViews = fc.views = {};


$.fn.skCalendar = function(options) {
	...
	if (typeof options == 'string') {
		...
		var res;
		this.each(function() {
			var calendar = $.data(this, 'skCalendar');
	...
	this.each(function(i, _element) {
		...
		element.data('skCalendar', calendar); // TODO: look into memory leak implications
		calendar.render();
	});
...
function MonthView(element, calendar) {
	...
	// imports
	AgendaView.call(t, element, calendar, 'agenda');
...
function AgendaEventRenderer() {
	...
	function slotSegHtml(event, seg) {
		...
		html +=
			...
			htmlEscape(event.service_name || '') +
			"</div>" +"<div class='fc-event-title'>" +
			htmlEscape(event.client_name || '') +
			"</div>" +
			"</div>" +
		...
		element
			...
			.click(function(ev) {
				if (isResizing) {
					...
					ev.stopImmediatePropagation(); // prevent skCalendar eventClick handler from being called
...
defaults.titleFormat.agendaList="MMM d[ yyyy]{ '&#8212;'[ MMM] d yyyy}";
