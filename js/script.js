			...
			success: function(data) {
				if(data==-2){ 
				 ...
				 $(".book_appointment").attr("onsubmit","return false;");
				  ...
				  var setVal=showsettings(business_id,$('.time').attr('action'));
...
$(".message").removeClass("alert").html(" ");
 ...
 function showsettings(businessid,action){
	var url=baseUrl+'bcalendar/getbsettings';
	var data={'businessid':businessid,'action':action};
	  $.post(url,data,function(data){ 
	     if(data==1){
		  var data=data+' hour';
		  }else{
			...
			success: function(data) { 
             ...
             var data=str.trim();			
				if(data==0 || data==-1){
				    ...
				    $(".book_appointment").attr("onsubmit","return false;");
				    var setVal=showsettings(business_id,action);
